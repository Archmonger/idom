...
def module_export_from_file(file: Path, max_depth: int) -> Set[str]:
    export_names, references = module_exports_in_source(file.read_text())
    if max_depth <= 0:
        logger.warning(f"Unable to resolve all exports for {file}")
    else:
        for ref in references:
            if urlparse(ref).scheme:  # is an absolute URL
                export_names.update(module_exports_from_url(ref, max_depth - 1))
            elif ref.startswith("."):
                ref = _resolve_relative_file_path(file, ref)
                export_names.update(module_export_from_file(ref, max_depth - 1))
            else:
                logger.warning(f"Did not resolve exports for unknown location {ref}")
    return export_names


def module_exports_from_url(url: str, max_depth: int) -> Set[str]:
    export_names, references = module_exports_in_source(requests.get(url).text)
    if max_depth <= 0:
        logger.warning(f"Unable to fully resolve all exports for {url}")
    else:
        for ref in map(_resolve_relative_url, references):
            export_names.update(module_exports_in_source(ref, max_depth - 1))
    return export_names


def module_exports_in_source(content: str) -> Tuple[Set[str], Set[str]]:
    names: Set[str] = set()
