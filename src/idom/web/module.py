...
from .utils import (
    find_js_module_exports_from_url,
...
def module_from_url(
    ...
    check_exports: bool = IDOM_DEBUG_MODE.current,
    check_exports_max_depth: int = 5,
) -> WebModule:
    return WebModule(
        source=url,
        ...
        export_names=(
            find_js_module_exports_from_url(url, check_exports_max_depth)
            if check_exports
            else None
        ),
    )
...
def module_from_template(
    ...
    return WebModule(
        ...
        default_fallback=fallback,
        export_names=(
            find_js_module_exports_from_url(f"{cdn}/{name}", check_exports_max_depth)
            if check_exports
            else None
        ),
    )
...
def module_from_file(
    ...
    return WebModule(
        ...
        default_fallback=fallback,
        export_names=(
            find_js_module_exports_in_source(source_file.read_text())
            if check_exports
            else None
        ),
    )
